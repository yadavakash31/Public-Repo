...
public  class EmployeeDAOImpl implements EmployeeDAO {
	...
	public void addAddress(Address add) {
		...
		template.save(add);
		//String query="insert into address (empref,presentAddress,addressId) values (?,?,?))";
		//template.find(query, key,add.getPresentAddress(),add.getAddressId());
		//Query query = session.createSQLQuery("insert into address (empref,presentAddress) values (?,?))");
		//query.setInteger(0, key);
		//query.setString(1, add.getPresentAddress());
		}
